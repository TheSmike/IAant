...
public class Chessboard {
	...
	public Chessboard(int N, int seedNumber) {
		this.N = N;
	...
	public int[][] getChessBoardNeighbourhood(Position center, int neighbourhood) {
		int[][] neighbMat = new int[neighbourhood * 2 + 1][neighbourhood * 2 + 1];

		int rOffset = center.getR() - neighbourhood;
		int cOffset = center.getC() - neighbourhood;
		...
		for (int r = center.getR() - neighbourhood; r <= center.getR() + neighbourhood; r++) {
			for (int c = center.getC() - neighbourhood; c <= center.getC() + neighbourhood; c++) {
				if (r < 0 || c < 0 || r >= N + 2 || c >= N + 2)
					neighbMat[r - rOffset][c - cOffset] = -N - 2;
				else
					neighbMat[r - rOffset][c - cOffset] = board[r][c];
			}
		}
